...
fn test_copy_diffstream_no_history_change() {
    ...
    let left = create_tree_with_copy_history(
        repo,
        &[
            (foo, "foo or maybe bar", &histories[foo]),
            (bar, "definitely bar", &histories[bar]),
        ],
    );
    ...
    let right = create_tree_with_copy_history(
        repo,
        &[
            (foo, "definitely foo", &histories[foo]),
            (bar, "definitely bar", &histories[bar]),
        ],
    );
...
fn test_copy_diffstream_copy() {
    ...
    let left = create_tree_with_copy_history(repo, &[(foo, "foo or maybe bar", &histories[foo])]);
...
fn test_copy_diffstream_rename() {
    ...
    let left = create_tree_with_copy_history(repo, &[(foo, "foo or maybe bar", &histories[foo])]);
    let foo_val = left.path_value(foo).unwrap();
    let right = create_tree_with_copy_history(repo, &[(bar, "definitely bar", &histories[bar])]);
    let bar_val = right.path_value(bar).unwrap();
    assert_eq!(
        collect_diffs(&left, &right),
        [
...
fn test_copy_diffstream_file_dir_mismatch() {
    ...
    let left = create_tree_with_copy_history(
        repo,
        &[(file_dir_path, "a file for now", &histories[file_dir_path])],
    );
...
fn test_copy_diffstream_distinct_histories() {
    ...
    let foo_history1 = CopyHistory {
        ...
        parents: vec![],
        salt: vec![1], // Use a salt to create distinct `CopyHistory`s
    };
    ...
    let left = create_tree_with_copy_history(repo, &[(foo, "first foo", &foo_history1)]);
    let foo_val1 = left.path_value(foo).unwrap();

    let right = create_tree_with_copy_history(repo, &[(foo, "second foo", &foo_history2)]);
...
fn test_copy_diffstream_dest_conflict() {
    ...
    let right1 = create_tree_with_copy_history(repo, &[(bar, "foobar", &histories[bar])]);
    let right2 = create_tree_with_copy_history(repo, &[(bar, "foobaz", &histories[bar])]);
    let right = right1
        .clone()
...
fn test_copy_diffstream_source_conflict() {
    ...
    let base_tree =
        create_tree_with_copy_history(repo, &[(foo, "foo or maybe bar", &histories[foo])]);
    let foo_val = base_tree.path_value(foo).unwrap();

    let left1 = create_tree_with_copy_history(repo, &[(foo, "Foo V1", &histories[foo])]);
    let left2 = create_tree_with_copy_history(repo, &[(foo, "foo vX", &histories[foo])]);
    let left = left1
        .clone()
    ...
    let right = create_tree_with_copy_history(repo, &[(bar, "Foo v1.X", &histories[bar])]);
    let expected = [
...
fn test_copy_diffstream_multiple_descendants() {
    ...
    let left = create_tree_with_copy_history(repo, &[(gru, "gru", &histories[gru])]);
    let gru_val = left.path_value(gru).unwrap();

    let right = create_tree_with_copy_history(
        repo,
        &[(bar, "bar", &histories[bar]), (qux, "qux", &histories[qux])],
    );
    let bar_val = right.path_value(bar).unwrap();
